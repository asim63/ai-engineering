...
class ChatPDF:
    ...
    def load_document(self):
        dir_path = r"Day-24-Advance-RAG-Patterns\/"
    ...
    def retrieve_result(self,query,db):
    ...
    def cross_encode(self,true_query,unique_results):
        ...
        reranked_docs = [(unique_results[i][0], float(rerank_scores[i])) for i in range(len(unique_results))]
        reranked_docs.sort(key=lambda x: x[1], reverse=True)
        
        # Sift the best top 3 high-precision matches
        return reranked_docs[:3]

    def retrieve_queries(self,query):
        response = chat(
            model="qwen3:8b",
            messages=[{
                "role":"user",
                "content": f"""
                    You are a query specialist. Rephrase the given query to similar other queries.
                    Query :
                    {query}
                    Answer:
                    "queries":[
        ...
        queries = json.loads(response["message"]["content"])["queries"]
        return queries
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if true_query.lower() == "/exit": break
        if true_query is None: break
        queries = chat_pdf.retrieve_queries(true_query)
        if not true_query in queries:
            queries.append(true_query)
        all_results = []
        for query in queries:
            results = chat_pdf.retrieve_result(query,db)
            if results:
        ...
        final_result = chat_pdf.cross_encode(true_query,unique_docs)

        context = "\n\n".join(doc.page_content for doc, _ in final_result)
        
        print("\n" + "="*20 + " Top Re-ranked Context " + "="*20)
        for i, (doc, score) in enumerate(final_result, start=1):
