...
import sqlite3
conn = sqlite3.connect(r"Day-27-LangGraph\agent_checkpoints.db",check_same_thread= False)
...
def run_with_approval(task:str, thread_id: str):
    os.chdir(r"D:\Projects\ai-engineering\Day-27-LangGraph")
    ...
    while state.next: #it is non empty unless the graph is paused. 
        last_message = state.values["messages"][-1]
        tool_calls = last_message.get("tool_calls", []) if isinstance(last_message, dict) else last_message.tool_calls
        ...
        for tool_call in tool_calls:
            if tool_call["function"]["name"] == "write_file":
                ...
                print(f"\n{'='*50}")
                print(f"APPROVAL NEEDED: write_file")
                print(f"  filepath: {args.get('filepath')}")
                print(f"  content preview: {args.get('content', '')[:200]}...")
                print(f"{'='*50}")

                approval = input("Approve this write? (yes/no): ").strip().lower()
        ...
        #resume the graph
        result = app.invoke(None, config = config) # note = continue from checkpoint
        state = app.get_state(config)
    return state.values 
...
if __name__ == "__main__":
    result = run_with_approval(
        "Write a short note saying 'Human-in-Loop ' to approval_test.txt",
